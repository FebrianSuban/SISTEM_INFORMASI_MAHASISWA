...
<head>
    ...
    <title>Biodata Publik - {{ $mahasiswa->nama_lengkap }}</title>
    ...
    <style>
        ...
        .print-wrap { max-width: 800px; margin: 20px auto; }
    </style>
</head>
<body class="bg-white text-gray-900">
    <div class="print-wrap bg-white border rounded-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                <img src="/images/642.jpg" alt="logo" class="h-14 w-auto object-contain" onerror="this.style.display='none'">
                <div>
                    <h1 class="text-lg font-bold text-[#2b0b5a]">INSTITUT DIGITAL EKONOMI LPKIA</h1>
                    <div class="text-sm text-gray-600">Biodata Mahasiswa</div>
                </div>
            </div>
            ...
            <div class="no-print">
                ...
                <a href="{{ route('mahasiswa.show', $mahasiswa) }}" class="ml-2 inline-flex items-center px-4 py-2 bg-gray-200 text-gray-700 rounded-md">Kembali</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2">
                <h2 class="text-2xl font-semibold text-[#2b0b5a] mb-3">{{ $mahasiswa->nama_lengkap }}</h2>
                ...
                <div class="mt-6 text-sm text-gray-600">Terdaftar: {{ $mahasiswa->created_at->format('d F Y H:i') }}</div>
            </div>
        </div>

        <div class="mt-6 text-xs text-gray-500">Dicetak pada: {{ now()->format('d F Y H:i') }}</div>
    </div>
